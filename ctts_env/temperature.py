...
logLambda_hart82 = np.array([-28.3, -26.0, -24.5, -23.6, -22.6, -21.8, -21.2]) #radiative loss
...
tab_logRadLoss = np.array([-28.3, -26.0, -24.5, -23.6, -23.1])#, -22.6])#, -21.8, -21.2])
spl_order = 1
logL = InterpolatedUnivariateSpline(logT_hart82, logLambda_hart82, k=spl_order)#extrapolated (ext=0)
logT_extrp = InterpolatedUnivariateSpline(tab_logRadLoss, tab_logT, k=spl_order)#extrapolated (ext=0)
logT_bound = InterpolatedUnivariateSpline(tab_logRadLoss, tab_logT, k=spl_order,ext=3)#bounded by min max of Hartmann (ext=3).
...
def compute_temp(r,rho,Tmax,B=0,type=0,Tmax_sec=0,mcol=[]):
	...
	if type==1: #normalize Hartmann to avg = Tmax
		...
		T[sec_col] = logRadLoss_to_T(np.log10(rl/rl.max()) + T_to_logRadLoss(Tmax_sec))
